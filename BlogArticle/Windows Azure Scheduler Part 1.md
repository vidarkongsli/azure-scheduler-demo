## The bird’s-eye view
Scheduler is a quite simple component in the quite vast set of services and features offered by Windows Azure. You should not confuse it with the WebJobs feature in Windows Azure Web Sites, which enables running programs or scripts in your web site. (Either on-demand or on a schedule). Scheduler lets you perform two types of actions (or three, depending on how you look at it):
...
From an architectural view, the core functionality that Scheduler offers is scheduling. It moves the scheduling logic from your application to a configurable component. Scheduler is an alternative to frameworks such as [Quartz.NET](http://www.quartz-scheduler.net/), Scheduled Tasks in Windows, and cron on UNIX systems. I would argue that the Azure Scheduler is very much like [Scheduled Tasks](https://developers.google.com/appengine/docs/python/config/cron) that has been available in Google AppEngine for several years. Also note that the Scheduler does not care about the application logic at all, on which platform it runs, or where it runs. As long as it is available using HTTP from Windows Azure or reads from a Windows Azure Storage Queue, it will work.
## Fundamentals
When you get introduced to Scheduler, you will see a few terms thrown around that might be a little confusing. I was a bit confused, anyway. The first term is “cloud service”, which, according to the [documentation](http://msdn.microsoft.com/en-us/library/windowsazure/dn528941.aspx)"represents an application". When you create a new scheduled job, you do not need to know about this term as it is set automatically based on the location you select. For example, when using “North Europe” as location, I got the name *CS-NorthEurope-scheduler*.

The most fundamental term in Scheduler is "job". A job represents an action to carried out at a certain time, and your jobs can put into job collections. There is no functional reason for putting jobs into collections. They can cross application and functional boundaries helter-skelter. You should nonetheless note that all jobs in a collection are in the same data center location, and quota enforcements apply based on collections.

Recurrence is about when the job executes. Either the job is set to occur at a specific time in the future, on a specific interval, or on a (more or less) advanced schedule. As the time of writing, the possibility of creating an advanced schedule is unavailable in the management console. The REST API offers more advanced options, but seems a little buggy. I will get back to this in part three of this series. 
## Choosing an action type
As I mentioned earlier, there are two main types of actions: HTTP/S and Storage Queue. If we get into details, we have to consider HTTP and HTTPS as two different types. I would argue that HTTP in general could have security ramifications that you should scrutinize. I would thus in most circumstances advice you to use HTTPS instead of HTTP. 
...
Having gone through some of the fundamentals and some considerations on which action type to use, let us try to set up a storage queue job. In this setup, I will show how to create an Azure WorkerRole that handles the messages that the Scheduler enters into the queue.
...
I have selected an action type of storage queue, and selected a queue in an existing storage account. You could also choose to create a new storage account and queue at this point. I have also clicked on the *Generate SAS token* which creates a token that the Scheduler needs to be able to access the queue. I have also selected to include a payload in the body of the message, which signals to the receiving application what it should to do. When it comes to the architecture of my solution I could also have chosen to have a separate queue for each type of job my solution does. Then I would not need to have anything in the message body. An empty message would suffice to trigger the given functionality.
...
Note that once created, there is no way to alter the job in the management console. You have to use the REST interface for that. I suspect this will be better in the released version.

We now have set up the Scheduler to create queue entries, but we have no application to process them. You can use any Azure Storage Explorer utility to peek into the queue to see if the entries appear. It would be a good idea to do so before moving on to create an application.
...
Select *File-->New Project* in Visual Studio. Then use the Windows Azure Cloud Service project template:
...
When prompted to enter the roles needed in my Cloud Service, create a Worker Role with name “EmailWorkerRole”:
...
The AccountName is the name of the storage account. In the example from above, this is *sojourn*. One can find the account key in the Azure Management Console by navigating to the given storage account and selecting *Manage access key* at the bottom of the page:
...
```
Note that we transfer the logs to storage once every five minutes. This means that we can experience up to five minutes delay before our log messages are available. For more efficient debugging, you might consider setting this to a lower value.
...
Publishing takes a few minutes, more often than not less than ten minutes. Once the application is up and running we can use a storage explorer to investigate our logs (I use [Azure Storage Explorer](http://azurestorageexplorer.codeplex.com/)). Here, we can see that our application has processed the message:
...
## In closing
I have explained the basic concepts of the Windows Azure Scheduler. Furthermore, I discussed what you can use it for, and what types of actions you can set up. Then, I went on to show an example on how to create a storage queue action and made a simple application to respond to these actions. Storage queue actions are especially good for processing tasks that might take a long time. Note that in our example, when popping messages off the queue, we made them invisible for five minutes. This means that we have five minutes to complete the processing. If your task might take longer, you should extend this time span.

Using storage queue actions requires some extra configuration which adds some complexity. You should probably use an HTTP/S action type instead, if your tasks execute fast and you have a web application set up. I will investigate such action types in my next post in this series on Windows Azure Scheduler.
